function template = fillClass(name, namespace, processed, classprops, inherited, superClassProps)
    ...
    if isa(class, 'file.Group') && (class.hasAnonGroups || class.hasAnonData)
        mixinMethodBlock = createMethodBlockForHasUnnamedGroupMixin(fullClassName);
        
        fullMethodBody = strjoin(...
            {fullMethodBody, ...
            '', ...
            mixinMethodBlock}, newline);
    end
...
function methodBlockStr = createMethodBlockForHasUnnamedGroupMixin(fullClassName)
    methodBlockStr = strjoin({...
        '% Override getGroupPropertyNames of matnwb.mixin.HasUnnamedGroups superclass', ...
        'methods (Static, Access = protected)', ...
        '    function propertyNames = getGroupPropertyNames()', ...
        '    % getGroupPropertyNames - Get property names for all unnamed groups of this type', ...
        sprintf('        propertyNames = matnwb.mixin.HasUnnamedGroups.getGroupPropertyNamesAcrossTypeHierarchy(''%s'');', fullClassName), ...
        '    end', ...
        'end'}, newline);
end
