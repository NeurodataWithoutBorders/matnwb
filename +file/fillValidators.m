    ...
    if prop.isConstrainedSet
        ...
        if ~isempty(prop.type) % Dataset class like e.g. VectorData
            ...
            datasetValidationLines = [...
                {fillDtypeValidation(name, prop.dtype, namespaceReg, addRewrap)}, ...
                {fillDimensionValidation(name, prop.shape)} ];
...
function fdvstr = fillDtypeValidation(name, type, namespaceReg, addRewrap)
    if nargin < 4; addRewrap = false; end
    if isstruct(type)
        if addRewrap
            skipCheckStr = strjoin({...
