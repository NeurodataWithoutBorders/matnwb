...
%GETPROPERTYHOOKS Return custom generator hooks for property validators/setters.
    ...
    if file.internal.isDescendantOf(typeName, 'DynamicTable', namespace) ...
    ...
    if file.internal.isDescendantOf(typeName, 'AlignedDynamicTable', namespace) ...
...
function tf = isSchemaDefinedAlignedDynamicTableCategory(prop, namespace)
    tf = isa(prop, 'file.Group') ...
        && ~prop.isConstrainedSet ...
        && ~isempty(prop.type) ...
        && file.internal.isDescendantOf(prop.type, 'DynamicTable', namespace);
end
