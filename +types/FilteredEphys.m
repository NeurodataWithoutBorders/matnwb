classdef FilteredEphys < types.NWBContainer
  ...
  methods %constructor
    function obj = FilteredEphys(varargin)
      ...
      obj.help = {'Ephys data from one or more channels that is subjected to filtering, such as for gamma or theta oscillations (LFP has its own interface). Filter properties should be noted in the ElectricalSeries'};
      ...
      if ~isempty(gn)
        for i=1:length(gn)
          gnm = gn{i};
          if isfield(obj, gnm)
            error('Naming conflict found in FilteredEphys object property name: ''%s''', gnm);
          else
            addprop(obj, gnm);
