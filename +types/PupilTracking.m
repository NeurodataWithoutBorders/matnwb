classdef PupilTracking < types.NWBContainer
  ...
  methods %constructor
    function obj = PupilTracking(varargin)
      ...
      obj.help = {'Eye-tracking data, representing pupil size'};
      ...
      if ~isempty(gn)
        for i=1:length(gn)
          gnm = gn{i};
          if isfield(obj, gnm)
            error('Naming conflict found in PupilTracking object property name: ''%s''', gnm);
          else
            addprop(obj, gnm);
