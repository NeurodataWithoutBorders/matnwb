classdef EventWaveform < types.NWBContainer
  ...
  methods %constructor
    function obj = EventWaveform(varargin)
      ...
      obj.help = {'Waveform of detected extracellularly recorded spike events'};
      ...
      if ~isempty(gn)
        for i=1:length(gn)
          gnm = gn{i};
          if isfield(obj, gnm)
            error('Naming conflict found in EventWaveform object property name: ''%s''', gnm);
          else
            addprop(obj, gnm);
