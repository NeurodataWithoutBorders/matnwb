classdef BehavioralTimeSeries < types.NWBContainer
  ...
  methods %constructor
    function obj = BehavioralTimeSeries(varargin)
      ...
      obj.help = {'General container for storing continuously sampled behavioral data.'};
      ...
      if ~isempty(gn)
        for i=1:length(gn)
          gnm = gn{i};
          if isfield(obj, gnm)
            error('Naming conflict found in BehavioralTimeSeries object property name: ''%s''', gnm);
          else
            addprop(obj, gnm);
