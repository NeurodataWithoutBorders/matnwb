...
% PLANESEGMENTATION - Results from image segmentation of a specific imaging plane. At least one of `image_mask`, `pixel_mask`, and `voxel_mask` is required.
%
% Required Properties:
...
% OPTIONAL PROPERTIES
properties
    image_mask; %  (VectorData) ROI masks for each ROI. Each image mask is the size of the original imaging plane (or volume) and members of the ROI are finite non-zero. At least one of `image_mask`, `pixel_mask`, and `voxel_mask` is required.
    pixel_mask; %  (VectorData) Pixel masks for each ROI: a list of indices and weights for the ROI. Pixel masks are concatenated and parsing of this dataset is maintained by the PlaneSegmentation. At least one of `image_mask`, `pixel_mask`, and `voxel_mask` is required.
    ...
    voxel_mask; %  (VectorData) Voxel masks for each ROI: a list of indices and weights for the ROI. Voxel masks are concatenated and parsing of this dataset is maintained by the PlaneSegmentation. At least one of `image_mask`, `pixel_mask`, and `voxel_mask` is required.
