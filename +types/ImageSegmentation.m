classdef ImageSegmentation < types.NWBContainer
  ...
  methods %constructor
    function obj = ImageSegmentation(varargin)
      ...
      obj.help = {'Stores groups of pixels that define regions of interest from one or more imaging planes'};
      ...
      if ~isempty(gn)
        for i=1:length(gn)
          gnm = gn{i};
          if isfield(obj, gnm)
            error('Naming conflict found in ImageSegmentation object property name: ''%s''', gnm);
          else
            addprop(obj, gnm);
