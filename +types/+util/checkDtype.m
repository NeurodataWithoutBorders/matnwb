...
%   to match the expected type.
    ...
    % Strict mode is for callers that are probing candidate dtypes, such as
    % constrained-type validation. In that context, a mismatch must throw so
    % the caller can keep trying other allowed types. The default report mode
    % is the public schema-validation boundary.
    ...
    try
        ...
        if isSchemaValidationException(exception)
            ...
            % In read context, schema violations are reported as warnings.
            % Preserve the original value so invalid-but-readable files are not
            % silently changed after validation reports the problem.
            value = originalValue;
        else
            rethrow(exception)
        end
...
function value = checkDtypeForCompoundDataset( ...
    ...
    % Validate subfield types using the caller's mode. This keeps constrained
    % type probes strict even when the mismatch is nested inside a compound
    % dtype, while normal validation still reports violations once at the
    % top-level checkDtype boundary.
    parentName = name;
