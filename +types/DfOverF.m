classdef DfOverF < types.NWBContainer
  ...
  methods %constructor
    function obj = DfOverF(varargin)
      ...
      obj.help = {'Df/f over time of one or more ROIs. TimeSeries names should correspond to imaging plane names'};
      ...
      if ~isempty(gn)
        for i=1:length(gn)
          gnm = gn{i};
          if isfield(obj, gnm)
            error('Naming conflict found in DfOverF object property name: ''%s''', gnm);
          else
            addprop(obj, gnm);
