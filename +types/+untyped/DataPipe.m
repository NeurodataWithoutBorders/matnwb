classdef DataPipe < types.untyped.DataStub
    %DATAPIPE Extendible incremental version of DataStub
    
    properties
        compression_level = 0; % DEFLATE level.  0 implies no compression.
    end
    
    methods
        function obj = DataPipe(filename, path, varargin)
            obj = obj@types.untyped.DataStub(filename, path);
            
            parser = inputParser;
            parser.addParameter('compression_level', 0,...
                @(val)isscalar(val) && isnumeric(val));
            parser.parse(varargin{:});
            
            obj.compression_level = floor(parser.Results.compression_level);
        end
        
        function append(data, dim)
        end
