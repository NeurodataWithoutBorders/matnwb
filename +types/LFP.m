classdef LFP < types.NWBContainer
  ...
  methods %constructor
    function obj = LFP(varargin)
      ...
      obj.help = {'LFP data from one or more channels. Filter properties should be noted in the ElectricalSeries'};
      ...
      if ~isempty(gn)
        for i=1:length(gn)
          gnm = gn{i};
          if isfield(obj, gnm)
            error('Naming conflict found in LFP object property name: ''%s''', gnm);
          else
            addprop(obj, gnm);
