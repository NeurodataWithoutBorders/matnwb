classdef BehavioralEpochs < types.NWBContainer
  ...
  methods %constructor
    function obj = BehavioralEpochs(varargin)
      ...
      obj.help = {'General container for storing behavorial epochs'};
      ...
      if ~isempty(gn)
        for i=1:length(gn)
          gnm = gn{i};
          if isfield(obj, gnm)
            error('Naming conflict found in BehavioralEpochs object property name: ''%s''', gnm);
          else
            addprop(obj, gnm);
