classdef Fluorescence < types.NWBContainer
  ...
  methods %constructor
    function obj = Fluorescence(varargin)
      ...
      obj.help = {'Fluorescence over time of one or more ROIs. TimeSeries names should correspond to imaging plane names'};
      ...
      if ~isempty(gn)
        for i=1:length(gn)
          gnm = gn{i};
          if isfield(obj, gnm)
            error('Naming conflict found in Fluorescence object property name: ''%s''', gnm);
          else
            addprop(obj, gnm);
