classdef UnitTimes < types.NWBContainer

  properties
  end

  methods %constructor
    function obj = UnitTimes(varargin)
      p = inputParser;
      p.KeepUnmatched = true;
      p.addParameter('groups', struct());
      p.parse(varargin{:});
      obj = obj@types.NWBContainer(varargin{:});
      obj.help = {'Estimated spike times from a single unit'};
      ...
      if ~isempty(gn)
        for i=1:length(gn)
          gnm = gn{i};
          if isfield(obj, gnm)
            error('Naming conflict found in UnitTimes object property name: ''%s''', gnm);
          else
            addprop(obj, gnm);
