...
% |*Device*|> object. The fields |description|, |serial_number| and |model| are 
% optional, but recommended. The |model| property can contain a <https://matnwb.readthedocs.io/en/latest/pages/neurodata_types/core/DeviceModel.html 
% |*DeviceModel*|> object which stores information about the device model. This 
% property can be useful when searching a set of NWB files or a data archive for 
% all files that use a specific device model (e.g., Neuropixels probe).
...
% The electrode table region created above is reused here. Wrapping it in a
% SoftLink writes a link to that region instead of a second copy of it, so
% both series refer to the same object in the file.
lfp_electrical_series = types.core.ElectricalSeries( ...
    ...
    'electrodes', types.untyped.SoftLink(electrode_table_region), ...
    'data_unit', 'volts');
...
decomp_series = types.core.DecompositionSeries(...
    ...
    'source_channels', types.untyped.SoftLink(electrode_table_region), ...
